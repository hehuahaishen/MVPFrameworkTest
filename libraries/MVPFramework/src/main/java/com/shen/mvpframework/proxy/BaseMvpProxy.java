...
/**
 * 代理--实现类，用来管理Presenter的生命周期，还有和view之间的关联
 */
public class BaseMvpProxy<V extends BaseMvpView, P extends BaseMvpPresenter<V>> implements PresenterProxyInterface<V, P>{
    ...
    public Bundle onSaveInstanceState() {
        ...
        if(mPresenter != null){
            ...
            mPresenter.onSaveInstanceState(presenterBundle);    // 回调Presenter
    ...
    /**
     * 设置Presenter的工厂类,
     * 这个方法只能在创建Presenter之前调用,也就是调用getMvpPresenter()之前，
    ...
    /**
     * 获取创建的Presenter                                   <p>
     * 果之前创建过，而且是以外销毁则从Bundle中恢复
     *
     * @return 指定类型的Presenter 如
     */
    @Override
